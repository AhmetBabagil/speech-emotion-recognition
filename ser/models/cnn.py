...
Girdi : [B, 1, n_mels, T]   (B=batch, 1=kanal, n_mels=frekans, T=zaman)
Çıktı : [B, num_classes] logit'ler (softmax UYGULANMAZ — CrossEntropyLoss
        logit bekler; olasılık gerektiğinde değerlendirme kodu softmax uygular)

Mimari: Dört konvolüsyon bloğu (2 x [Conv-BN-ReLU] + MaxPool) ve ardından
global ortalama havuzlama + doğrusal sınıflandırıcı. Spektrogram bir "görüntü"
gibi işlenir: alçak katmanlar yerel zaman-frekans desenlerini (enerji
kenarları, harmonik çizgiler), üst katmanlar daha soyut birleşimleri öğrenir.

Global havuzlama önemli bir tasarım kararıdır: sınıflandırıcı başlığını zaman
boyutundan bağımsız kılar — [B, C, H', W'] hangi genişlikte olursa olsun havuz
sonrası [B, C] elde edilir. Böylece klip uzunluğu (T) değişse bile model
çalışmaya devam eder ve Flatten'lı tasarımlara göre çok daha az parametre gerekir.
"""
...
class ConvBlock(nn.Module):
    ...
    * ReLU(inplace=True): aktivasyonu yerinde yazarak bellekten tasarruf eder.
    * İki konvolüsyonu ARKA ARKAYA koymak (VGG deseni), tek büyük çekirdeğe göre
...
class MelCNN(nn.Module):
    """Konvolüsyonel gövde + global ortalama havuz + dropout'lu doğrusal başlık."""
