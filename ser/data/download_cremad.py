...
#
# Neden dosya adı bu kadar önemli? CREMA-D'nin etiketi VE konuşmacı kimliği yalnızca dosya adında durur. Orijinal adlar her iki yöntemde de korunduğundan, oyuncu id'sine dayanan konuşmacı-bağımsız bölme sorunsuz çalışır.
...
def _count_wavs(audiowav: Path) -> int:
    # Klasördeki .wav sayısı (klasör yoksa 0) — hem 'zaten indirilmiş mi?' kontrolünde hem indirme sonrası doğrulamada kullanılır.
...
def download_cremad(dest: str = "data/raw/cremad", method: str = "hf") -> Path:
    ...
    #
    # İdempotenttir: yeterli sayıda WAV zaten diskteyse hiçbir şey indirmez — betiği tekrar tekrar çalıştırmak güvenlidir. AudioWAV yolunu döndürür.
...
def _download_via_hf(audiowav: Path) -> Path:  # Hugging Face aynasından WAV baytlarını indirip diske yazar.
    try:
...
def _run(cmd: list[str], env: dict | None = None) -> None:
    # Bir alt komutu loglayarak çalıştırır; hata kodunda exception fırlatır (check=True). Kullanıcı, hangi git komutunun koştuğunu logda görebilir.
    log.info("$ %s", " ".join(cmd))
