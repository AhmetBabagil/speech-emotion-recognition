...
#
# Kısa kuram: Ham dalga formu, duygu gibi "tınısal" bilgiyi doğrudan göstermez. STFT ile ses küçük pencerelere bölünüp frekans uzayına taşınır; mel filtre bankası bu frekans eksenini insan kulağının duyarlılığına göre (alçak frekanslarda ince, yüksekte kaba) yeniden ölçekler; logaritma (dB) ise insan gürlük algısına uyan sıkıştırmayı yapar. Sonuç, CNN'in "görüntü gibi" işleyebileceği 2 boyutlu bir [n_mels, T] matristir.
...
def fixed_num_frames(num_samples: int, hop_length: int) -> int:
    # ``num_samples`` örnek için librosa'nın üreteceği mel karesi sayısı (``center=True`` varsayılanıyla).
    #
    # Formülün nedeni: center=True iken sinyal iki uçtan yansıtılarak doldurulur ve kare sayısı floor(N / hop) + 1 olur. Bu sayı önceden bilinirse her spektrogram aynı genişliğe kırpılıp/doldurulup sorunsuz batch'lenebilir.
...
def log_mel_spectrogram(wav: np.ndarray, feature_cfg, sample_rate: int) -> np.ndarray:
    ...
    #
    # ``feature_cfg`` bir :class:`ser.config.FeatureConfig` nesnesidir; STFT/mel parametreleri oradan gelir (tek doğruluk kaynağı ilkesi).
    import librosa
...
def fix_frames(spec: np.ndarray, num_frames: int, *, random_crop: bool = False,
    ...
    #
    # fix_length'in spektrogram karşılığıdır: eğitimde rastgele, değerlendirmede merkez kırpma; kısaysa doldurma.
    t = spec.shape[1]
    if t == num_frames:
        return spec
    if t < num_frames:
...
def standardize(spec: np.ndarray, eps: float = 1e-5) -> np.ndarray:
    ...
    #
    # Her spektrogram KENDİ ortalama ve std'siyle normalize edilir (global veri istatistikleriyle değil). Bu, kayıt seviyesi/mikrofon farklarını örnek bazında siler ve global istatistik tutma derdinden kurtarır. ``eps``, tamamen sabit (std=0) girdilerde 0'a bölünmeyi önler.
    mu = spec.mean()
    sd = spec.std()
...
def spec_augment(spec: np.ndarray, freq_mask: int, time_mask: int,
    ...
    #
    # Fikir (Park ve ark., 2019): spektrogramın rastgele bir şeridini silmek, modeli tek bir frekans bandına ya da tek bir ana ezberlenmiş anına bağımlı olmaktan caydırır — dropout'un girdi-uzayındaki karşılığı gibi düşünülebilir. Maske değeri 0'dır; standardize sonrası 0 zaten ortalamaya denk gelir. Orijinal makaledeki çoklu maske yerine tek maske kullanılır (küçük veri kümelerinde aşırı bozmamak için "hafif" versiyon).
    rng = rng or np.random.default_rng()
