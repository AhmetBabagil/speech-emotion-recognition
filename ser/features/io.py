# Ses G/Ç katmanı: herhangi bir WAV'ı hedef örnekleme frekansında mono float32 diziye yükler ve sabit örnek sayısına getirir (doldur ya da kırp).
#
# Bu iki işlem ayrı bir modüldedir çünkü hem CNN yolu (spektrogramdan önce) hem wav2vec2 yolu (doğrudan dalga formu) aynı yükleme/uzunluk mantığını paylaşır.
...
def load_audio(path: str, target_sr: int) -> np.ndarray:
    ...
    #
    # librosa kullanılır (arkada soundfile/audioread'i sarar); böylece CREMA-D'nin WAV'ları ve bizim MELD ffmpeg çıkarımımızın ürettiği WAV'lar Windows/Linux fark etmeksizin aynı şekilde açılır. Dönen değer kabaca [-1, 1] aralığında 1 boyutlu float32 dizidir.
    #
    # Hata felsefesi: bozuk/okunamayan TEK dosya koca bir eğitim koşusunu çökertmemeli. Bu yüzden hata durumunda uyarı loglanır ve kısa bir SESSİZ klip döndürülür (yukarıda sabit uzunluğa zaten doldurulacaktır). Binlerce dosyalık gerçek veride bu dayanıklılık şarttır.
    import librosa
...
def fix_length(
    ...
    *,
    ...
    #
    # Neden sabit uzunluk? Tensörleri batch'lemek için tüm örneklerin aynı boyutta olması gerekir. Eğitimde uzun klipler RASTGELE konumdan kırpılır — her epoch'ta klibin farklı bir bölümü görülür, bu bedava bir veri çoğaltmadır. Değerlendirmede ise MERKEZDEN kırpılır ki sonuç deterministik olsun.
    n = wav.shape[0]
    if n == num_samples:
